...
class DebugScanAgent(Agent):
    def __init__(self,
        ...
        with self.lock:
            self.log_dir_path = f"{BASE_PATH}/log/debugscan/{self.model_name}/{self.bug_type}/{self.language}/{self.project_name}/{time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime())}-{agent_id}"
