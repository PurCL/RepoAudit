...
class SliceScanAgent(Agent):
    def __init__(
        ...
        # State of the agent
        self.state = SliceScanState(
            self.seed_function, self.seed_values, self.call_depth, self.is_backward
        )
        return
                ...
                if ext_val_type == "Output Value":
                    ...
                    for callee_function in callee_functions:
                        ...
                        for call_site_node in call_sites:
                            ...
                            ret_values = (
                                self.ts_analyzer.get_return_values_in_single_function(
                                    callee_function
                                )
                            )
    ...
    def start_scan_sequential(self) -> None:
        ...
        while True:
            ...
            for (
                ...
                is_mergeable = False
                for (
                    worklist_slice_context,
                    worklist_function_id,
                    worklist_seed_set,
                ) in worklist:
                    if (
                        delta_slice_context != worklist_slice_context
                        or delta_function_id != worklist_function_id
                    ):
                        continue
                    worklist_seed_value = list(worklist_seed_set)[0]
                    if (
                        delta_seed_value.label == ValueLabel.RET
                        and worklist_seed_value.label == ValueLabel.RET
                    ) or (
                        delta_seed_value.line_number == worklist_seed_value.line_number
                    ):
                ...
                if not is_mergeable:
                    worklist.append(
                        (delta_slice_context, delta_function_id, {delta_seed_value})
                    )
    ...
    def start_scan(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self.max_neural_workers) as executor:
            while worklist:
                futures = [
                    executor.submit(self.__process_item, item) for item in worklist
                ]
                    ...
                    with self.lock:
                        for (
                            ...
                            for i, (
                                ...
                                if (
                                    delta_slice_context == wl_slice_context
                                    and delta_function_id == wl_function_id
                                ):
