...
cases = {
    "cpv-1": {
        ...
        "File": "../benchmark/Cpp/cpv-1/src/http/ngx_http_request.c",
        "Type": "Buffer Overflow",
    },
    "cpv-2": {
        "Function": "ngx_decode_base64_internal",
        "File": "../benchmark/Cpp/cpv-2/src/core/ngx_string.c",
        "Type": "Buffer Overflow",
    },
    "cpv-3": {
        "Function": "ngx_http_trace_handler",
        "File": "../benchmark/Cpp/cpv-3/src/http/ngx_http_request.c",
        "Type": "Buffer Overflow",
    },
    "cpv-4": {
        "Function": "ngx_http_set_browser_cookie",
        "File": "../benchmark/Cpp/cpv-4/src/http/ngx_http_core_module.c",
        "Type": "Buffer Overflow",
    },
    "cpv-5": {
        "Function": "ngx_http_get_last_ip_variable",
        "File": "../benchmark/Cpp/cpv-5/src/http/ngx_http_variables.c",
        "Type": "Null Pointer Dereference",
    },
    "cpv-8": {
        "Function": "ngx_mail_pop3_user",
        "File": "../benchmark/Cpp/cpv-8/src/mail/ngx_mail_pop3_handler.c",
        "Type": "Buffer Overflow",
    },
    "cpv-9": {
        "Function": "ngx_black_list_remove",
        "File": "../benchmark/Cpp/cpv-9/src/core/ngx_cycle.c",
        "Type": "Use After Free",
    },
    "cpv-10": {
        "Function": "ngx_http_process_prefer",
        "File": "../benchmark/Cpp/cpv-10/src/http/ngx_http_request.c",
        "Type": "Use After Free",
    },
    "cpv-11": {
        "Function": "ngx_init_cycle",
        "File": "../benchmark/Cpp/cpv-11/src/core/ngx_cycle.c",
        "Type": "Use After Free",
    },
    "cpv-12": {
        "Function": "ngx_sendfile_r",
        "File": "../benchmark/Cpp/cpv-12/src/os/unix/ngx_linux_sendfile_chain.c",
        "Type": "Buffer Overflow",
    },
    "cpv-13": {
        "Function": "ngx_mail_pop3_pass",
        "File": "../benchmark/Cpp/cpv-13/src/mail/ngx_mail_pop3_handler.c",
        "Type": "Null Pointer Dereference",
    },
    "cpv-14": {
        "Function": "ngx_http_set_exten",
        "File": "../benchmark/Cpp/cpv-14/src/http/ngx_http_core_module.c",
        "Type": "Buffer Overflow",
    },
    "cpv-15": {
        "Function": "ngx_decode_base64_internal",
        "File": "../benchmark/Cpp/cpv-15/src/core/ngx_string.c",
        "Type": "Buffer Overflow",
    },
    "cpv-17": {
        "Function": "ngx_mail_smtp_noop",
        "File": "../benchmark/Cpp/cpv-17/src/mail/ngx_mail_smtp_handler.c",
        "Type": "Use After Free",
    },
}
...
class SampleScanAgent(Agent):
    def __init__(
        ...
        self.intra_detector = IntraFunctionDetector(
            self.bug_type,
    ...
    def start_scan_squential(self) -> None:
        ...
        for seed_value, is_backward in self.initial_seeds:
            ...
            initial_seeds_in_functions[seed_function.function_id].append(
                (seed_value, is_backward)
            )
        ...
        # dump to log
        output_seed_value_strs = [
            str(output_seed_value)
            for (output_seed_value, is_backward) in self.sampled_seeds
        ]
        ...
        for seed_value, is_backward in self.sampled_seeds:
            ...
            for file_name, line_number in self.state.bug_report_lines.values():
                if (
                    file_name == seed_value.file
                    and line_number == seed_value.line_number
                ):
                    is_analyzed = True
                    break
            if is_analyzed:
            ...
            self.logger.print_console(
                f"{len(self.state.bug_report_lines)} bug(s) was/were detected in total."
            )
            self.logger.print_console(
    ...
    def start_scan(self) -> None:
        ...
        with tqdm(total=total_seeds, desc="Processing Seeds", unit="seed") as pbar:
            with ThreadPoolExecutor(max_workers=self.max_neural_workers) as executor:
                futures = [
                    executor.submit(
                        self.__process_seed_parallel, seed_value, is_backward, index
                    )
    ...
    def __process_seed_parallel(
        ...
        self.logger.print_console("---------------------------------------------------")
        self.logger.print_console(
            "Step III & IV: Start the detection upon inlined slice..."
        )
        self.logger.print_console("Value info: ", str(seed_value))
        ...
        for slice_inliner_input in slice_inliner_inputs:
            ...
            self.logger.print_console(
                "Step III: Start the inlining...",
                f"{cnt + 1}/{len(slice_inliner_inputs)}",
            )
            self.logger.print_console(
                "---------------------------------------------------\n"
            )
