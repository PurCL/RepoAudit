    ...
    def get_if_statements(
        ...
        for if_statement_node in if_statement_nodes:
            ...
            condition_str = source_code[
                sub_node_types[1].start_byte : sub_node_types[1].end_byte
            ]
            condition_start_line = (
                source_code[: sub_node_types[1].start_byte].count("\n") + 1
            )
            condition_end_line = (
                source_code[: sub_node_types[1].end_byte].count("\n") + 1
            )
            true_branch_start_line = (
                source_code[: sub_node_types[3].start_byte].count("\n") + 1
            )
            true_branch_end_line = (
                source_code[: sub_node_types[3].end_byte].count("\n") + 1
            )

            if "else_clause" in [
                sub_node.type for sub_node in sub_node_types
            ] or "elif_clause" in [sub_node.type for sub_node in sub_node_types]:
                else_branch_start_line = (
                    source_code[: sub_node_types[4].start_byte].count("\n") + 1
                )
        ...
        for loop_node in loop_nodes:
            ...
            for loop_child_node in loop_node.children:
                if loop_child_node.type == ":":
                    header_line_start = (
                        source_code[: loop_node.start_byte].count("\n") + 1
                    )
                    header_line_end = (
                        source_code[: loop_child_node.start_byte].count("\n") + 1
                    )
                    header_str = source_code[
                        loop_node.start_byte : loop_child_node.start_byte
                    ]
