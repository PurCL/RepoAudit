...
class SeedSelector(LLMTool):
    def __init__(
        ...
        self.prompt_file = (
            f"{BASE_PATH}/prompt/{language}/samplecan/{bug_type}_seed_selector.json"
        )
        return
                ...
                list(
                    set(
                        [
                            str(
                                value.line_number
                                - input.seed_function.start_line_number
                                + 1
                            )
        ...
        if "No" in final_answer.lower():
            ...
            for number in numbers:
                ...
                for value in input.seed_list:
                    if (
                        value.line_number
                        == line_number + input.seed_function.start_line_number - 1
                    ):
