...
class DebugSliceAnalyzerInput(LLMToolInput):
    def __init__(
        self, err_message: str, debug_seed: Value, debug_slice: str, request_str: str
    ) -> None:
        """
    ...
    def __hash__(self) -> int:
        return hash(
            (self.err_message, str(self.debug_seed), self.debug_slice, self.request_str)
        )
...
class DebugSliceAnalyzer(LLMTool):
    def __init__(
        ...
        self.prompt_file = (
            f"{BASE_PATH}/prompt/{language}/debugscan/debug_slice_analyzer.json"
        )
        return
