...
class DebugRequestFormulatorInput(LLMToolInput):
    def __init__(self, error_message: str, lined_function_code: str, error_file_path: str, request: str) -> None:
        """
...
class DebugRequestFormulator(LLMTool):
    def __init__(self, model_name: str, temperature: float, language: str, max_query_num: int, logger: Logger) -> None:
        ...
        self.prompt_file = f"{BASE_PATH}/prompt/{language}/debugscan/debug_request_formulator.json"
        return

    def _get_prompt(self, debug_request_formulator_input: DebugRequestFormulatorInput) -> str:
        ...
        prompt = prompt.replace("<ERRMSG>", debug_request_formulator_input.error_message)
        prompt = prompt.replace("<LINED_FUNCTION_CODE>", debug_request_formulator_input.lined_function_code)
        return prompt

    def _parse_response(self, response: str, debug_request_formulator_input: DebugRequestFormulatorInput) -> DebugRequestFormulatorOutput:
        """
        ...
        if match:
            ...
            debug_seed = Value(expr, int(line_number), ValueLabel.SINK, debug_request_formulator_input.error_file_path)
            output = DebugRequestFormulatorOutput(debug_seed, debug_request_formulator_input.request)
            return output
        else:
